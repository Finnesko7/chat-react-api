...
$(document).ready(function () {
  var roomId = $('#room-id').data('id');
  window.Echo.channel('notification.' + roomId).listen('.message.send', function (message) {
    appendMessage(message.author, message.message, message.sendTime);
  });

  function appendMessage(author, text, sendTime) {
    var message = '<div class="p-6 bg-white border-b border-gray-200"><p>' + author + '</p><p>' + text + '</p><p>' + sendTime + '</p></div>';
